            ...
            try {
                ...
                while (true) {
                    ...
                    try {
                        ...
                        if (hr == 0 && desc.DedicatedVideoMemory > 0) {
                            var l = desc.AdapterLuid;
                            // Format must match Windows perf counter instance exactly
                            var instance = string.Format("luid_0x{0:X8}_0x{1:X8}_phys_0", (uint)l.HighPart, l.LowPart);
                            // Clean name: DXGI sometimes has trailing nulls or control chars, trim
                            var name = (desc.Description ?? "GPU").TrimEnd('\0', ' ');
                            if (string.IsNullOrWhiteSpace(name)) name = "GPU" + i;
                            result.Add(new GpuInfo { InstanceName = instance, Name = name, VideoMemory = desc.DedicatedVideoMemory });
                        }
                    } finally {
